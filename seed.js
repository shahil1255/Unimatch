...
async function seed() {
  try {
    ...
    console.log('✅ Connected to MongoDB Atlas. Re-seeding with MIO scores...');
    ...
    for (const country of countries) {
      ...
      for (let i = 0; i < 60; i++) { // Increased to 60 per country
        ...
        universities.push({
          ...
          minMio: Math.floor(Math.random() * 45) + 50 // Randomized MIO between 50-95
        });
      }
      
      if (universities.length >= 500) {
          await University.insertMany(universities);
          console.log(`...Inserted ${universities.length} entries`);
          universities = [];
      }
    }
    ...
    console.log(`✅ Success! Seeded ${count} universities with MIO scores across ${countries.length} countries.`);
    process.exit(0);
  } catch (err) {
